...
class TrafficFlowTopApplication(BaseModel):
    ...
    The DPI catalogue key is ``(category_id << 16) | application_id``.
    ``application_name``/``category_name`` are null when the catalogue does not
    cover the entry.
    """
...
def enrich_traffic_flow_statistics_dpi_names(
    ...
    The join is a read-only annotation. Original IDs and byte counts remain
    unchanged, and catalogue misses remain null.
    """
    application_names = {
        entry["id"]: entry["name"]
    ...
    for application in statistics.top_applications:
        ...
        if application.application_id is not None and application.category_id is not None:
            compound_id = (application.category_id << 16) | application.application_id
            application_name = application_names.get(compound_id)
